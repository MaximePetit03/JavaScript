...
export class Player {
  constructor(id, name, skinPath, position) {
    ...
    this.currentPostionX = position[0];
    this.currentPostionY = position[1];
  ...
  update(updateData) {
    this.oldPositionX = this.currentPostionX;
    this.oldPositionY = this.currentPostionY;

    // Update authoritative position
    [this.currentPostionX, this.currentPostionY] = updateData.position;
  ...
  interpolate(alpha) {
    const dist = Math.sqrt(
      Math.pow(this.currentPostionX - this.oldPositionX, 2) +
        Math.pow(this.currentPostionY - this.oldPositionY, 2),
    );

    if (dist < 0.001) {
      this.renderX = this.currentPostionX;
      this.renderY = this.currentPostionY;
      return;
    }

    this.renderX =
      this.oldPositionX + (this.currentPostionX - this.oldPositionX) * alpha;
    this.renderY =
      this.oldPositionY + (this.currentPostionY - this.oldPositionY) * alpha;
  }
